...
def plot_tsp(p, x_coord, W, W_val, W_target, title="default"):
    ...
    nx.draw_networkx_edges(
        G, pos, edgelist=target_pairs, alpha=1, width=1, edge_color="r"
    )
    p.set_title(title)
    return p
...
def plot_tsp_heatmap(p, x_coord, W_val, W_pred, title="default"):
    ...
    node_color = ["g"] + ["b"] * (
        len(x_coord) - 1
    )  # Green for 0th node, blue for others
...
def plot_predictions(
    x_nodes_coord, x_edges, x_edges_values, y_edges, y_pred_edges, num_plots=3
):
    """
    ...
    for f_idx, idx in enumerate(np.random.choice(len(y), num_plots, replace=False)):
        ...
        plot_tsp(
            ...
            W_val,
            W_target,
            "Groundtruth: {:.3f}".format(W_to_tour_len(W_target, W_val)),
        )
...
def plot_predictions_beamsearch(
    x_nodes_coord, x_edges, x_edges_values, y_edges, y_pred_edges, bs_nodes, num_plots=3
):
    """
    ...
    for f_idx, idx in enumerate(np.random.choice(len(y), num_plots, replace=False)):
        ...
        plot_tsp(
            ...
            W_val,
            W_bs,
            "Beamsearch: {:.3f}".format(W_to_tour_len(W_bs, W_val)),
        )
        plt.show()
